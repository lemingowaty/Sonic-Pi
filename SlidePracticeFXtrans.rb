...
live_loop :mel1a do
  density 2 do
    with_transpose transring.tick do
      ...
      n1 = play :a1, attack: 1, release: 4, note_slide: 1, amp: 2
      density 2 do |n2x|
    ...
    density 2 do |x|
      with_transpose x*12 do
        with_synth :pluck do
          with_fx :slicer, phase: 0.65, amp: 1 do
            with_fx :reverb, amp: 0.75, room: 1 do
              sl = play :g0, release: 4, sustain: 2 , attack: 0.5, note_slide: 2
              sl.control("note", :a1)
              sleep 2
              density 2 do
                sleep 1
...
live_loop :hardbass, sync: "/live_loop/mel1a" do
  with_fx :krush do
    density 2 do |x|
      s = sample :bass_hard_c, amp: 0.25, rate: 1-(x*0.5), finish: 0.25, release: 0.125
      10.times do |i|
        sleep 0.1
        if i>5
