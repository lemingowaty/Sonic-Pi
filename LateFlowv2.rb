    ...
    4.times do |x|
      density 4 do |z|
        4.times do |y|
          ...
          play mel1ring.tick, release: 0.25
          density 2 do |p|
            play mel1ring.look - ( x!=3 && y==3 && z>=2 ?  -14 : 4 ), release: 0.5*(p+1), pitch: p
            sleep 0.5
          end
          play mel1ring.tick, release: 0.25
          play mel1ring.look - ( x==3 && y>=2 && z==3 ?  -2 : -16 ), on: (x==3 && y>=2 && z>=2), release:0.75
          sleep 0.25
          density 2 do
            play mel1ring.look - ( x==3 && z==3 &&y>=2 ?  -2 : -16-12 )+12 , on: (x==3 && y>=2 && z==3), release: 0.25
            sleep 0.25
          end
        end
      end
